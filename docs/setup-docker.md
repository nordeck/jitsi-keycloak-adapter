...
Some customizations are needed for `Nginx`'s site configuration. Therefore mount
the following custom `meet.conf` file to `jitsi-web` container to overwrite the
default one:
